...
class Gardian(object):
	...
	def lock_function(self, *dargs, lock_name=None, alarm_snapshot=None, trigger_alarm=True, post_alarm=None, **dkwargs):

		def __lock(locked_function):
			id_ = self.add_security_gate(locked_function,gate_id=lock_name)

			def __verification(*args,**kwargs):
				if self.decision_function(*dargs,**dkwargs):
					return locked_function(*args,**kwargs)
